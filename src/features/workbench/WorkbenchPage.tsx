            ...
            <div className="grid gap-4">
              <section id="workbench-resumo" className="scroll-mt-4 grid gap-4 md:grid-cols-4">
                <MetricCard
                  size="compact"
                  label="Clientes"
                  value={workbench.counts.assignedClients}
                />
                <MetricCard
                  size="compact"
                  label="Atenção"
                  value={workbench.counts.portfoliosNeedingAttention}
                />
                <MetricCard
                  size="compact"
                  label="Em aberto"
                  value={workbench.counts.openReviewItems}
                />
                <MetricCard
                  size="compact"
                  label="Alta atenção"
                  value={workbench.counts.highSeverityReviewItems}
                />
              </section>
        ...
        <>
          <section className="grid gap-4 md:grid-cols-4">
            ...
            <MetricCard
              size="compact"
              label="Portfólios"
              value={charts.dataQuality.sourceCounts.portfolios}
            />
            <MetricCard
              size="compact"
              label="Alertas"
              value={charts.dataQuality.sourceCounts.alerts}
            />
            <MetricCard
              size="compact"
              label="Pacotes"
              value={charts.dataQuality.sourceCounts.reportPackages}
            />
          </section>
...
function LineChart({ points }: { points: AdvisorChartBundle["charts"]["bookValueTrend"] }) {
  if (points.length === 0) {
    return <ChartEmptyState tone="info">Nenhum histórico disponível.</ChartEmptyState>;
  }
...
function SeverityTimeline({
  ...
  if (points.length === 0) {
    return <ChartEmptyState tone="info">Nenhum alerta no período.</ChartEmptyState>;
  }
...
function ReportPipeline({ points }: { points: AdvisorChartBundle["charts"]["reportPipeline"] }) {
  if (points.length === 0) {
    return <ChartEmptyState tone="info">Nenhum pacote no período.</ChartEmptyState>;
  }
...
function WorkbenchAging({ points }: { points: AdvisorChartBundle["charts"]["workbenchAging"] }) {
  ...
  if (visible.length === 0) {
    return <ChartEmptyState tone="info">Nenhum item aberto na fila.</ChartEmptyState>;
  }
...
function StaleBacklog({ items }: { items: AdvisorChartBundle["charts"]["staleDataBacklog"] }) {
  if (items.length === 0) {
    return <ChartEmptyState tone="info">Nenhum backlog de atualização.</ChartEmptyState>;
  }
...
function NeedsAttentionTable({
  ...
  if (items.length === 0) {
    return <ChartEmptyState tone="info">Nenhum cliente priorizado nos filtros.</ChartEmptyState>;
  }
